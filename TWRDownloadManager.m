...
@interface TWRDownloadManager () <NSURLSessionDelegate,
...
- (instancetype)init {
  ...
  if (self) {
    ...
    backgroundConfiguration = [NSURLSessionConfiguration
        backgroundSessionConfigurationWithIdentifier:[[NSBundle mainBundle]
                                                         bundleIdentifier]];
...
- (void)downloadFileForURL:(NSString *)urlString
  ...
  if (![self fileDownloadCompletedForUrl:urlString]) {
    ...
    TWRDownloadObject *downloadObject =
        [[TWRDownloadObject alloc] initWithDownloadTask:downloadTask
...
- (void)cancelAllDownloads {
  [self.downloads enumerateKeysAndObjectsUsingBlock:^(
                      id key, TWRDownloadObject *download, BOOL *stop) {
    if (download.completionBlock) {
      download.completionBlock(NO);
    }
    [download.downloadTask cancel];
...
- (NSArray *)currentDownloads {
  ...
  [self.downloads enumerateKeysAndObjectsUsingBlock:^(
                      id key, TWRDownloadObject *download, BOOL *stop) {
    [currentDownloads
        addObject:download.downloadTask.originalRequest.URL.absoluteString];
  }];
...
- (void)URLSession:(NSURLSession *)session
  ...
  CGFloat remainingTime =
      [self remainingTimeForDownload:download
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (success) {
    ...
    NSURL *cacheDirectoryUrlPath = [self cachesDirectoryUrlPath];
    destinationLocation =
        (download.directoryName)
            ? [[cacheDirectoryUrlPath
                  URLByAppendingPathComponent:download.directoryName]
                  URLByAppendingPathComponent:download.fileName]
            : [cacheDirectoryUrlPath
                  URLByAppendingPathComponent:download.fileName];
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    localNotification.alertBody = [NSString
        stringWithFormat:@"%@ has been downloaded", download.friendlyName];
...
- (CGFloat)remainingTimeForDownload:(TWRDownloadObject *)download
  ...
  NSTimeInterval timeInterval =
      [[NSDate date] timeIntervalSinceDate:download.startDate];
...
- (BOOL)createDirectoryNamed:(NSString *)directory {
  ...
  NSString *targetDirectory =
      [cachesDirectory stringByAppendingPathComponent:directory];
...
- (BOOL)fileExistsForUrl:(NSString *)urlString
             inDirectory:(NSString *)directoryName {
  return [self fileExistsWithName:[urlString lastPathComponent]
...
- (BOOL)fileExistsWithName:(NSString *)fileName
  ...
  if ([[NSFileManager defaultManager]
          fileExistsAtPath:[[cachesDirectory
...
- (BOOL)deleteFileForUrl:(NSString *)urlString
             inDirectory:(NSString *)directoryName {
  return [self deleteFileWithName:[urlString lastPathComponent]
                      inDirectory:directoryName];
}
...
- (BOOL)deleteFileWithName:(NSString *)fileName
  ...
  NSURL *fileLocation;
  if (directoryName) {
    fileLocation = [[[self cachesDirectoryUrlPath]
        URLByAppendingPathComponent:directoryName]
        URLByAppendingPathComponent:fileName];
  } else {
    fileLocation =
        [[self cachesDirectoryUrlPath] URLByAppendingPathComponent:fileName];
  }
...
- (void)cleanDirectoryNamed:(NSString *)directory {
  ...
  for (NSString *file in
    ...
    [fm removeItemAtPath:[directory stringByAppendingPathComponent:file]
                   error:&error];
  }
}
    ...
    if ([downloadTasks count] == 0) {
      if (self.backgroundTransferCompletionHandler != nil) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
          // Call the completion handler to tell the system that there are no
          // other background transfers.
          completionHandler();
